...
export const fetch = async (accessToken: string, { projectID, itemID }: { projectID: string; itemID: string }): Promise<types.Version[]> => {
  ...
  const data = body.data.map(utils.parseVersion);
  try {
    assertType<types.Version[]>(data);
  } catch (e) {
